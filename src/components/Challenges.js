...
const TaskRow = (props) => {
    const { item, getChallenges, id, index, lastIndex, content, updateChallenges, stratId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/strategy/" + id + "/" + stratId + "/challenges/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    return (
        <>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Challenge details'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are the challenges?</Label>
                            {
                                update ?
...
function Challenges(props) {
    ...
    let { content, auth, getChallenges, updateChallenges, stratId } = props
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/strategy/" + auth.uid + "/" + stratId + "/challenges/" + item.order;
    ...
    return (
        <>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">CHALLENGES</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow stratId={stratId} updateChallenges={updateChallenges} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getChallenges={getChallenges} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                <div className="wis-model__header">Add new challenge</div>
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are the challenges?</Label>
