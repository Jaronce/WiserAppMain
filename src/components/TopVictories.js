...
const TaskRow = (props) => {
    const { item, getTopVictories, id, index, lastIndex, content, updateTopVictories, insId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/review/"+ id + "/" + insId + "/topVictories/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    return (
        <>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Victory details'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What's your victory?</Label>
                            {
                                update ?
...
function TopVictories(props) {

    let { content, auth, getTopVictories, updateTopVictories, insId } = props
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/review/"+ auth.uid + "/" + insId + "/topVictories/" + item.order;
    ...
    return (
        <>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">MY TOP 3 VICTORIES</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow insId={insId} updateTopVictories={updateTopVictories} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getTopVictories={getTopVictories} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                <div className="wis-model__header">Add new victory</div>
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What is your victory?</Label>
