...
const TaskRow = (props) => {
    const { item, getTopAwaitInsight, id, index, lastIndex, content, updateTopAwaitInsight, insId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/review/"+ id + "/" + insId + "/topAwaitInsight/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    return (
        <>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Take away insights'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What's your take away?</Label>
                            {
                                update ?
...
function TopAwaitIsight(props) {

    let { content, auth, getTopAwaitInsight, updateTopAwaitIsight, insId } = props
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/review/"+ auth.uid + "/" + insId + "/topAwaitInsight/" + item.order;
    ...
    return (
        <>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">TOP TAKE AWAY/INSIGHT</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow insId={insId} updateTopAwaitIsight={updateTopAwaitIsight} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getTopAwaitInsight={getTopAwaitInsight} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                <div className="wis-model__header">Add new Take away</div>
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What is your new Take away?</Label>
                            <Input checked={true} onChange={handleChange} className="wis-model__group-input" type="text" name="header" id="header" />
                        </FormGroup>
                    </Form>
                </div>
                <div className="wis-model__footer">
