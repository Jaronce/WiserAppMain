...
const TaskRow = (props) => {
    const { item, getOthers, id, index, lastIndex, content, updateOthers, insId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/inspiration/"+ id + "/" + insId + "/others/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
                ...
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What's your other related goals?</Label>
                            {
                                update ?
...
function InspOthers(props) {

    let { content, auth, getOthers, updateOthers, insId } = props
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/inspiration/"+ auth.uid + "/" + insId + "/others/" + item.order;
        ...
        <>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">OTHER RELATED GOALS</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow insId={insId} updateOthers={updateOthers} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getOthers={getOthers} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                ...
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your other related goals?</Label>
                            <Input value={header} onChange={handleChange} className="wis-model__group-input" type="text" name="header" id="header" />
                        </FormGroup>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">Goal Details?</Label>
