...
const TaskRow = (props) => {
    const { item, getHabits, id, index, lastIndex, content, updateHabits, week, execId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/execution/" + id + "/" + execId + "/" + week + "/habits/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    return (
        <>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Habits details'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your new habits?</Label>
                            {
                                update ?
...
function ExecHabits(props) {

    let { content, auth, week, getHabits, updateHabits, execId } = props
    ...
    const handleSubmit = async () => {
        let habitsCount = 0
        const fbpath1 = "/counters/" + auth.uid + "/exec/" + execId + "/" + week;
        ...
        if (item) {
            const fbpath = "/execution/" + auth.uid + "/" + execId + "/" + week + "/habits/" + item.order;
        ...
        <>
            <div className="main-content__card">
                <div className="main-content__card-header">
                    ...
                    <span id='execution' className="main-content__card-header-title">Execution</span>
                </div>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">DAILY HABITS</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow execId={execId} week={week} updateHabits={updateHabits} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getHabits={getHabits} />
                                ))
                            :
                                <tr draggable className="main-content__card-table-tab-body-row">
                                    <th scope="row" className="main-content__card-table-tab-body-row-head" ><FontAwesomeIcon icon={faArrowsAlt} /></th>
                                    <td className="main-content__card-table-tab-body-row-main"> Not Filled Yet</td>
                                </tr>
                            }
                        </tbody>
                    </Table>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                <div className="wis-model__header">Add new habit</div>
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your new habits?</Label>
