...
const TaskRow = (props) => {
    const { item, getPriority, id, index, lastIndex, content, updatePriority } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/who/priority/" + id + "/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    return (
        <>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Priority details'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your top three priorities?</Label>
                            {
                                update ?
...
function Priority(props) {
    ...
    let { content, auth, getPriority, updatePriority } = props
    ...
    const handleSubmit = async (e) => {
        ...
        if (item) {
            const fbpath = "/who/priority/" + auth.uid + "/" + item.order;
        ...
        <>
            <div className="main-content__card">
                <div className="main-content__card-header">
                    ...
                    <span id='priority' className="main-content__card-header-title">Top Three Priorities</span>
                </div>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">Priorities</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow updatePriority={updatePriority} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getPriority={getPriority} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                <div className="wis-model__header">Add new priority</div>
