...
const TaskRow = (props) => {
    const { item, getResources, id, index, lastIndex, content, updateResources, stratId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/strategy/" + id + "/" + stratId + "/resources/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    return (
        <>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Resources details'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your resources?</Label>
                            {
                                update ?
...
function Resources(props) {
    ...
    let { content, auth, getResources, updateResources, stratId } = props
    ...
    const [tooltipResOpen, setTooltipResOpen] = useState(false);

    const toggleRes = () => setTooltipResOpen(!tooltipResOpen);
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/strategy/" + auth.uid + "/" + stratId + "/resources/" + item.order;
    ...
    return (
        <>
            <div className='flex-col-2' md='6'>
                            ...
                            <Table className="main-content__card-table-tab">
                                <thead className="main-content__card-table-tab-head">
                                    <tr>
                                        ...
                                        <th className="main-content__card-table-tab-head-title">RESOURCES</th>
                                        <th>
                                            <FontAwesomeIcon id='resourcesInfo' style={{ color: '#E37263', fontSize: '1.5rem', cursor: 'pointer' }} icon={faInfoCircle} />
                                            <Tooltip style={{color: 'white', padding: '0.5rem', backgroundColor: 'rgba(227, 114, 99, 0.8)', width: '30rem', marginBottom: '1rem', borderRadius: '0.5rem'}} placement="top" isOpen={tooltipResOpen} target="resourcesInfo" toggle={toggleRes} >
                                                What are some key resources that will support you in your goal? (people, things, knowledge, etc)
                                            </Tooltip>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody className="main-content__card-table-tab-body">
                                    { content.length > 0 ? 
                                        content.map((item, index) => 
                                            ...
                                            <TaskRow updateResources={updateResources} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getResources={getResources} />)
                                    :
                ...
                <div className="wis-model__header">Add new resource</div>
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your resources?</Label>
