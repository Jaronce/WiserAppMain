...
const TaskRow = (props) => {
    const { item, getFears, id, index, lastIndex, content, updateFears, insId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/inspiration/"+ id + "/" + insId + "/fearss/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    const handleSave = () => {
        ...
        const fbpath = "/inspiration/"+ id + "/" + insId + "/fears/" + item.order;
        if(state && state !== item) {
            fbConfig.database()
                .ref(fbpath)
                .update({
                ...
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What's your fears?</Label>
                            {
                                update ?
                        ...
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">Fear Detail</Label>
                            {
                                update ?
...
function InspFears(props) {

    let { content, auth, getFears, updateFears, insId } = props
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/inspiration/"+ auth.uid + "/" + insId + "/fears/" + item.order;
    ...
    return (
        <>  
            <div className="main-content__card-header">
                    ...
                    <span id='goals' className="main-content__card-header-title">Fear Managing</span>
                </div>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">Define your fear</th>
                                { content.length > 0 ? <th className="main-content__card-table-tab-head-other">PREVENT</th> : '' }
                                { content.length > 0 ? <th className="main-content__card-table-tab-head-other">REPAIR</th> : '' }
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow insId={insId} updateFears={updateFears} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getFears={getFears} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                <div className="wis-model__header">Add new fear</div>
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What is your fears?</Label>
                            <Input value={header} onChange={handleChange} className="wis-model__group-input" type="text" name="header" id="header" />
                        </FormGroup>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">Fear Details?</Label>
                            <Input value={details} onChange={handleChange} className="wis-model__group-input wis-model__group-input-large" type="text" name="details" id="details" />
                        </FormGroup>
                    </Form>
                </div>
                <div className="wis-model__footer">
