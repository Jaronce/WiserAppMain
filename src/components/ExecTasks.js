...
const TaskRow = (props) => {
    const { item, getTasks, id, index, lastIndex, content, updateTasks, week, execId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/execution/" + id + "/" + execId + "/" + week + "/tasks/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    return (
        <>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Tasks details'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your new tasks?</Label>
                            {
                                update ?
...
function ExecTasks(props) {

    let { content, auth, week, getTasks, updateTasks, execId } = props
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/execution/" + auth.uid + "/" + execId + "/" + week + "/tasks/" + item.order;
    ...
    const {header, appointement} = state
    return (
        <>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">TASKS TO DO</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow execId={execId} week={week} updateTasks={updateTasks} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getTasks={getTasks} />
                                ))
                            :
