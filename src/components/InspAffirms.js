...
const TaskRow = (props) => {
    const { item, getAffirms, id, index, lastIndex, content, updateAffirms, insId } = props
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/inspiration/"+ id + "/" + insId + "/affirmations/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
                ...
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd"> Create a powerful affirmation</Label>
                            {
                                update ?
...
function InspAffirms(props) {

    let { content, auth, getAffirms, updateAffirms, insId, id } = props
    ...
    const handleSubmit = async () => {
        ...
        if (item) {
            const fbpath = "/inspiration/"+ auth.uid + "/" + insId + "/affirmations/" + item.order;
    ...
    return (
        <>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">POWERFUL AFFIRMATIONS</th>
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow insId={insId} updateAffirms={updateAffirms} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getAffirms={getAffirms} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                <div className="wis-model__header">Add new affirmation</div>
                <div className="wis-model__body">
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What is your affirmations?</Label>
