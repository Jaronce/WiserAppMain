...
import { faChevronCircleDown, faArrowsAlt, faEllipsisH, faCheck, faPlusCircle, faArrowUp, faArrowDown, faHourglassHalf } from '@fortawesome/free-solid-svg-icons'

const TaskRow = (props) => {
    const { item, getActions, id, index, lastIndex, content, updateActions, week, execId } = props
    ...
    const handleChange = (e) => {
        if( e.target.id === 'done') {
            setState({
                ...state,
                done: e.target.value === 'true' ? true : false
            })
    ...
    const handleDelete = async () => {
        ...
        const fbpath = "/execution/" + id + "/" + execId + "/" + week + "/actions/" + item.order;
        if(item) {
            fbConfig.database()
                .ref(fbpath)
                .remove()
    ...
    const handleSave = () => {
        ...
        if(state && state !== item) {
            fbConfig.database()
                .ref(fbpath)
                .update({
                    order: item.order, header: state.header, appointement: state.appointement, done: state.done, date: state.date
                }).then(res => {
                    setUpdate(false)
                    setModal(false)
    ...
    return (
        <>
            { 
                item.header != null ? 
                    <tr className="main-content__card-table-tab-body-row">
                        ...
                        <td className="main-content__card-table-tab-body-row-main">{item.done ? <FontAwesomeIcon icon={faCheck} /> : <FontAwesomeIcon icon={faHourglassHalf} /> }{item.header}</td>
                        <td className="main-content__card-table-tab-body-row-other">{moment(item.appointement).format('dddd')}</td>
                        <td className="main-content__card-table-tab-body-row-other">{moment(item.appointement).calendar()}</td>
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} >
                <div className="wis-model__header">{ update ? 'Modify item' : 'Actions details'}</div>
                <div className="wis-model__body">
                    ...
                    <Form>
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="gratatudeAdd">What are your new actions?</Label>
                            {
                                update ?
                        ...
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="appointementDet">Appointment</Label>
                            {
                                update ?
                                    <Input onChange={handleChange} defaultValue={item.appointement} className="wis-model__group-input" type="date" name="appointement" id="appointement" />
                                    :
                                    <Input value={item.appointement} className="wis-model__group-input" type="text" name="appointement" id="appointement" />
                            }
                        </FormGroup>

                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="doneDet">Done?</Label>
                            {
                                update ?
                                    <Input onChange={handleChange} defaultValue={item.done} className="wis-model__group-input" type="select" name="done" id="done">
                                        <option value={true}>Yes</option>
                                        <option value={false}>No</option>
                                    </Input>
                                    :
                                    <Input value={item.done ? 'Yes' : 'No'} className="wis-model__group-input" type="text" name="done" id="done" />
                            }
                        </FormGroup>
...
function ExecActions(props) {

    let { content, auth, week, getActions, updateActions, execId } = props
    ...
    const handleSubmit = async () => {
        let actionsCount = 0
        const fbpath1 = "/counters/" + auth.uid + "/" + execId + "/exec/" + week;
        ...
        actionsCount = counters.actions
        actionsCount++

        const date = new Date()
        const item = {
            header: state.header,
            appointement: state.appointement,
            done: state.done,
            order: actionsCount,
            date: date.getTime()
        }
        ...
        if (item) {
            const fbpath = "/execution/" + auth.uid + "/" + execId + "/" + week + "/actions" + item.order;
            await fbConfig.database()
            .ref(fbpath)
            .set({
                order: item.order,
                header: item.header,
                appointement: item.appointement,
                done: item.done,
                date: item.date
            })
            .then(snapshot => {
    ...
    const {header, appointement, done} = state
    return (
        <>
                    ...
                    <Table className="main-content__card-table-tab">
                        <thead className="main-content__card-table-tab-head">
                            <tr>
                                ...
                                <th className="main-content__card-table-tab-head-title">3 KEY ACTIONS</th>
                                { content.length > 0 ? <th className="main-content__card-table-tab-head-other">APPOINTMENT</th> : '' }
                                { content.length > 0 ? <th className="main-content__card-table-tab-head-other">TIME</th> : '' }
                        ...
                        <tbody className="main-content__card-table-tab-body">
                            { 
                                ...
                                content.map((item, index) => (
                                    <TaskRow execId={execId} week={week} updateActions={updateActions} content={content} lastIndex={content.length - 1} index={index} key={item.order} id={state.id} item={item} getActions={getActions} />
                                ))
                            :
            ...
            <div className="wis-model" style={{display: modal ? 'block' : 'none'}} isOpen={modal} toggle={toggle} >
                ...
                <div className="wis-model__body">
                    <Form>
                        ...
                        <FormGroup className="wis-model__group">
                            <Label className="wis-model__group-label" for="appointement">Appointment?</Label>
                            <Input value={appointement} onChange={handleChange} className="wis-model__group-input" type="date" name="appointement" id="appointement" />
                        </FormGroup>
                    </Form>
                </div>
                <div className="wis-model__footer">
